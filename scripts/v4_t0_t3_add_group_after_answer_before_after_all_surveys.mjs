...
const ADDED_GROUP = process.env.ADDED_GROUP || '2';
const PAUSE_MS = Number(process.env.PAUSE_MS || 1800);
const OUT_DIR = process.env.OUT_DIR || '/tmp/v4_t0_t3_all_surveys';
...
async function main() {
  ...
  const addedGroup = await getUserGroupByTitle(token, ADDED_GROUP);
  if (!baseGroup?.id || !addedGroup?.id) throw new Error(`Cannot resolve groups ${BASE_GROUP}, ${ADDED_GROUP}`);

  // T0: ensure +12 in group 1 and not in 2
  const fixes = [];
  ...
  let inAdded = await isUserInGroup(token, user.id, addedGroup.id, TARGET_EMAIL);
  if (inAdded) {
    const rem = await removeUserFromGroup(token, user.id, addedGroup.id);
    if (!rem.ok) throw new Error(`T0 cannot remove from added group: ${rem.status}`);
    fixes.push(`removed_from_${ADDED_GROUP}`);
  }
  inBase = await isUserInGroup(token, user.id, baseGroup.id, TARGET_EMAIL);
  inAdded = await isUserInGroup(token, user.id, addedGroup.id, TARGET_EMAIL);
  if (!inBase || inAdded) throw new Error(`T0 hard-fail: expected in ${BASE_GROUP}=true and in ${ADDED_GROUP}=false`);
  ...
  // T2: add group 2 after answer
  const add2 = await addUserToGroup(token, user.id, addedGroup.id);
  if (!add2.ok) throw new Error(`T2 cannot add to group ${ADDED_GROUP}: ${add2.status}`);
  const inBaseAfter = await isUserInGroup(token, user.id, baseGroup.id, TARGET_EMAIL);
  const inAddedAfter = await isUserInGroup(token, user.id, addedGroup.id, TARGET_EMAIL);
  if (!inBaseAfter || !inAddedAfter) throw new Error(`T2 hard-fail: expected in ${BASE_GROUP}=true and in ${ADDED_GROUP}=true`);
  ...
  for (const row of surveys) {
    ...
    const userBefore = await exportXlsx({
      token, surveyId, filters: userFilters, key: `survey_${surveyId}_rev_${revisionId}_v4_user_before`, resultsWithGroups: false,
    });
    const userAfter = await exportXlsx({
      token, surveyId, filters: userFilters, key: `survey_${surveyId}_rev_${revisionId}_v4_user_after`, resultsWithGroups: true,
    });
    const cycleBefore = await exportXlsx({
      token, surveyId, filters: cycleFilters, key: `survey_${surveyId}_rev_${revisionId}_v4_cycle_before`, resultsWithGroups: false,
    });
    const cycleAfter = await exportXlsx({
      token, surveyId, filters: cycleFilters, key: `survey_${surveyId}_rev_${revisionId}_v4_cycle_after`, resultsWithGroups: true,
    });
    ...
    row.checks = {
      ...
      userExport_after_hasAddedGroup: userAfter.ok ? vals.some((v) => hasTokenLike(v, ADDED_GROUP)) : null,
      ...
      modelInference: userAfter.ok
        ? (vals.some((v) => hasTokenLike(v, BASE_GROUP)) && vals.some((v) => hasTokenLike(v, ADDED_GROUP))
            ? 'actual_1_plus_2'
  ...
  const report = {
    ...
    addedGroup: ADDED_GROUP,
    t0: { userId: user.id, groups: groupsT0, fixesApplied: fixes, inBase: true, inAdded: false },
    t2: { groupsAfterAdd: groupsT2, inBase: inBaseAfter, inAdded: inAddedAfter },
    t3: { groups: groupsT3 },
    surveys,
  };

  const outFile = path.join(OUT_DIR, 'v4_t0_t3_result.json');
