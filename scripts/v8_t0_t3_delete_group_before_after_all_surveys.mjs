...
const DELETED_GROUP = process.env.DELETED_GROUP || '6';
const NO_GROUP_TEXT = process.env.NO_GROUP_TEXT || 'Без группы';
const PAUSE_MS = Number(process.env.PAUSE_MS || 1800);
const OUT_DIR = process.env.OUT_DIR || '/tmp/v8_t0_t3_all_surveys';
...
async function main() {
  ...
  let deletedGroup = await getGroupByTitle(token, DELETED_GROUP);
  if (!deletedGroup?.id) {
    throw new Error(`Group ${DELETED_GROUP} is not found. V8 requires existing group to delete.`);
  }

  // T0: ensure +9 only in group 6
  const fixes = [];
  const groups = await getAllGroups(token);
  for (const g of groups) {
    const inGroup = await isUserInGroup(token, user.id, g.id, TARGET_EMAIL);
    if (!inGroup) continue;
    if (Number(g.id) === Number(deletedGroup.id)) continue;
  ...
  const inDeletedAtT0 = await isUserInGroup(token, user.id, deletedGroup.id, TARGET_EMAIL);
  if (!inDeletedAtT0) {
    const add = await addUserToGroup(token, user.id, deletedGroup.id);
    if (!add.ok) throw new Error(`T0 cannot add to group ${DELETED_GROUP}: ${add.status}`);
    fixes.push(`added_to_${DELETED_GROUP}`);
  }
  const t0InDeleted = await isUserInGroup(token, user.id, deletedGroup.id, TARGET_EMAIL);
  if (!t0InDeleted) throw new Error(`T0 hard-fail: ${TARGET_EMAIL} not in group ${DELETED_GROUP}`);
  ...
  // T2: delete group 6
  const deleteRes = await deleteGroup(token, deletedGroup.id);
  if (!deleteRes.ok) throw new Error(`T2 cannot delete group ${DELETED_GROUP}: ${deleteRes.status}`);
  const groupAfterDelete = await getGroupByTitle(token, DELETED_GROUP);
  if (groupAfterDelete?.id) throw new Error(`T2 hard-fail: group ${DELETED_GROUP} still exists after delete`);
  ...
  for (const row of surveys) {
    ...
    const userBefore = await exportXlsx({
      token, surveyId, filters: userFilters, key: `survey_${surveyId}_rev_${revisionId}_v8_user_before`, resultsWithGroups: false,
    });
    const userAfter = await exportXlsx({
      token, surveyId, filters: userFilters, key: `survey_${surveyId}_rev_${revisionId}_v8_user_after`, resultsWithGroups: true,
    });
    const cycleBefore = await exportXlsx({
      token, surveyId, filters: cycleFilters, key: `survey_${surveyId}_rev_${revisionId}_v8_cycle_before`, resultsWithGroups: false,
    });
    const cycleAfter = await exportXlsx({
      token, surveyId, filters: cycleFilters, key: `survey_${surveyId}_rev_${revisionId}_v8_cycle_after`, resultsWithGroups: true,
    });
    ...
    row.checks = {
      ...
      userExport_after_hasDeletedGroupToken: userAfter.ok ? vals.some((v) => hasTokenLike(v, DELETED_GROUP)) : null,
      userExport_after_hasNoGroupText: userAfter.ok
        ? vals.some((v) => normalizeToken(v) === normalizeToken(NO_GROUP_TEXT))
        : null,
      userExport_after_hasEmptyValue: userAfter.ok ? vals.some((v) => String(v).trim() === '') : null,
      ...
      modelInference: userAfter.ok
        ? (vals.some((v) => hasTokenLike(v, DELETED_GROUP))
            ? 'historical_old_value'
            : vals.some((v) => normalizeToken(v) === normalizeToken(NO_GROUP_TEXT))
            ? 'actual_no_group_text'
  ...
  const report = {
    ...
    t0: { userId: user.id, inDeletedGroup: t0InDeleted, fixesApplied: fixes, deletedGroupId: deletedGroup.id },
    t2: { groupDeleted: !groupAfterDelete },
    surveys,
  };

  const outFile = path.join(OUT_DIR, 'v8_t0_t3_result.json');
