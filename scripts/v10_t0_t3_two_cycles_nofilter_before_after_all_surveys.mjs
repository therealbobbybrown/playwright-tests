...
const OUT_DIR = process.env.OUT_DIR || '/tmp/v10_t0_t3_all_surveys';
...
function inferModel(groupValues) {
  ...
  if (has1 && has2) return 'per_row_or_mixed';
...
async function main() {
  ...
  // T3: current state should be group 2 (remove all others, keep only 2)
  const fixes = [];
  ...
  for (const g of groups) {
    ...
    if (!rem.ok) throw new Error(`T3 cannot remove from group ${g.title}: ${rem.status}`);
  ...
  if (!in2) {
    ...
    if (!add.ok) throw new Error(`T3 cannot add to group ${GROUP_CYCLE2}: ${add.status}`);
  ...
  for (const surveyId of SURVEY_IDS) {
    ...
    const pair = revisionsWithTarget.length >= 2 ? revisionsWithTarget.slice(-2) : revisions.slice(-2);
    const row = {
      surveyId,
      revisions: pair.map((r) => ({ id: r.id, alias: r.alias ?? null })),
      precondition: {
        twoRevisionsWithTargetAnswers: revisionsWithTarget.length >= 2,
    ...
    if (pair.length < 2) {
      row.checks = { error: 'less_than_two_revisions_total' };
      surveys.push(row);
      continue;
    }

    const filters = {
      revisionsIds: [pair[0].id, pair[1].id],
      usersIds: [],
      userGroupsIds: [],
      departmentsIds: [],
    };
    ...
    const before = await exportXlsx({
      ...
      surveyId,
      filters,
      key: `survey_${surveyId}_rev_${pair[0].id}_${pair[1].id}_v10_before`,
    ...
    const after = await exportXlsx({
      ...
      surveyId,
      filters,
      key: `survey_${surveyId}_rev_${pair[0].id}_${pair[1].id}_v10_after`,
  ...
  const report = {
    ...
    cycle2Group: GROUP_CYCLE2,
    t3CurrentGroup: GROUP_CYCLE2,
    t3FixesApplied: fixes,
    surveys,
  };

  const outFile = path.join(OUT_DIR, 'v10_t0_t3_result.json');
