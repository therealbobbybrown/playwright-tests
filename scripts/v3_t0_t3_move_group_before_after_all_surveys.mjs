...
const TARGET_EMAIL = process.env.TARGET_EMAIL || 'qaseed+11@example.org';
const FROM_GROUP = process.env.FROM_GROUP || '1';
const TO_GROUP = process.env.TO_GROUP || '2';
const PAUSE_MS = Number(process.env.PAUSE_MS || 1800);
const OUT_DIR = process.env.OUT_DIR || '/tmp/v3_t0_t3_all_surveys';
...
async function main() {
  ...
  const fromGroup = await getUserGroupByTitle(token, FROM_GROUP);
  const toGroup = await getUserGroupByTitle(token, TO_GROUP);
  if (!fromGroup?.id || !toGroup?.id) {
    throw new Error(`Cannot resolve groups: from=${FROM_GROUP} to=${TO_GROUP}`);
  }

  // T0: ensure user in FROM and not in TO
  const t0Fixes = [];
  let inFrom = await isUserInGroup(token, user.id, fromGroup.id, TARGET_EMAIL);
  if (!inFrom) {
    const add = await addUserToGroup(token, user.id, fromGroup.id);
    if (!add.ok) throw new Error(`T0 cannot add to FROM group: ${add.status}`);
    t0Fixes.push(`added_to_${FROM_GROUP}`);
  }
  let inTo = await isUserInGroup(token, user.id, toGroup.id, TARGET_EMAIL);
  if (inTo) {
    const rem = await removeUserFromGroup(token, user.id, toGroup.id);
    if (!rem.ok) throw new Error(`T0 cannot remove from TO group: ${rem.status}`);
    t0Fixes.push(`removed_from_${TO_GROUP}`);
  }

  inFrom = await isUserInGroup(token, user.id, fromGroup.id, TARGET_EMAIL);
  inTo = await isUserInGroup(token, user.id, toGroup.id, TARGET_EMAIL);
  if (!inFrom || inTo) {
    throw new Error(`T0 hard-fail: expected in ${FROM_GROUP}=true and in ${TO_GROUP}=false`);
  }
  ...
  // T2: move FROM -> TO
  const rem = await removeUserFromGroup(token, user.id, fromGroup.id);
  if (!rem.ok) throw new Error(`T2 cannot remove from FROM group ${FROM_GROUP}: ${rem.status}`);
  const add = await addUserToGroup(token, user.id, toGroup.id);
  if (!add.ok) throw new Error(`T2 cannot add to TO group ${TO_GROUP}: ${add.status}`);

  const inFromAfterMove = await isUserInGroup(token, user.id, fromGroup.id, TARGET_EMAIL);
  const inToAfterMove = await isUserInGroup(token, user.id, toGroup.id, TARGET_EMAIL);
  if (inFromAfterMove || !inToAfterMove) {
    throw new Error(`T2 hard-fail: expected in ${FROM_GROUP}=false and in ${TO_GROUP}=true`);
  }
  const groupsT2 = await getAllUserGroupsByUserId(token, user.id);

  // T3: exports before/after feature (post-move state)
  for (const surveyResult of surveys) {
    if (!surveyResult.revision?.id) continue;
    const surveyId = surveyResult.surveyId;
    const revisionId = surveyResult.revision.id;
    ...
    const userBefore = await exportXlsx({
      ...
      surveyId,
      filters: userFilters,
      key: `survey_${surveyId}_rev_${revisionId}_v3_user_before`,
    ...
    const userAfter = await exportXlsx({
      ...
      surveyId,
      filters: userFilters,
      key: `survey_${surveyId}_rev_${revisionId}_v3_user_after`,
    ...
    const cycleBefore = await exportXlsx({
      ...
      key: `survey_${surveyId}_rev_${revisionId}_v3_cycle_before`,
      resultsWithGroups: false,
    });
    const cycleAfter = await exportXlsx({
      ...
      surveyId,
      filters: cycleFilters,
      key: `survey_${surveyId}_rev_${revisionId}_v3_cycle_after`,
      resultsWithGroups: true,
    });

    const userAfterValues = userAfter.ok ? userAfter.parsed.groupValues : [];
    ...
    surveyResult.checks = {
      ...
      userExport_after_hasFromGroup: userAfter.ok ? userAfterValues.some((v) => hasTokenLike(v, FROM_GROUP)) : null,
      userExport_after_hasToGroup: userAfter.ok ? userAfterValues.some((v) => hasTokenLike(v, TO_GROUP)) : null,
      ...
      modelInference: userAfter.ok
        ? (userAfterValues.some((v) => hasTokenLike(v, TO_GROUP))
            ? 'actual'
            : userAfterValues.some((v) => hasTokenLike(v, FROM_GROUP))
            ? 'historical'
            : 'no_user_row')
        : 'no_export',
    };
  ...
  const report = {
    ...
    t0: {
      userId: user.id,
      groups: groupsT0,
      fixesApplied: t0Fixes,
      inFrom: true,
      inTo: false,
    },
    t2: {
      groupsAfterMove: groupsT2,
      inFrom: inFromAfterMove,
      inTo: inToAfterMove,
    },
    t3: {
      groups: groupsT3,
  ...
  const outFile = path.join(OUT_DIR, 'v3_t0_t3_result.json');
