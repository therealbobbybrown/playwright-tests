// tests/functional/profile/profile-review-scoreonly-textonly.spec.js
// C####: Сотрудник видит только текстовую характеристику без числовой оценки (enableOnlyCustomCharacteristics)
//
...
// Дополнительно включает enableOnlyCustomCharacteristics=true, чтобы скрыть
// числовую оценку и показать ТОЛЬКО текстовую характеристику.
...
test.describe(
  "Профиль сотрудника — только текстовая характеристика (enableOnlyCustomCharacteristics)",
  {
    tag: [
      "@profile",
      "@performance-review",
  ...
  () => {
    ...
    test.beforeAll(async ({ request }) => {
      ...
      if (existing) {
        ...
        // Включить enableOnlyCustomCharacteristics
        ...
        console.log(
          `Найден существующий PR с характеристикой: id=${charPrId}, "${charPrTitle}", ` +
            `label="${charLabel}", enableOnlyCustomCharacteristics=true`,
        );
        return;
      }
      ...
      // Остановить PR (enableOnlyCustomCharacteristics применяется ПОСЛЕ остановки и проверки)
      ...
      // Включить enableOnlyCustomCharacteristics ПОСЛЕ проверки characteristic
      savedSettings = await saveSettings(adminAPI, charPrId);
      await patchStatisticsSettings(adminAPI, charPrId, {
        enableOnlyCustomCharacteristics: true,
      });
    ...
    test.beforeEach(async () => {
      markAsUITest(MODULES.PROFILE, "ScoreOnly Text-Only Characteristic");
    });
    ...
    test(
      ...
      async ({ userAuth: userPage, prAPI }, testInfo) => {
        ...
        await test.step(
          "Текстовая характеристика видна, числовая оценка скрыта",
          async () => {
            const label = await reviewPage.getFinalScoreLabel(charPrTitle);
            expect(
              label,
              `Текстовая характеристика должна отображаться (ожидается "${charLabel}")`,
            ).toBe(charLabel);

            const score = await reviewPage.getFinalScoreValue(charPrTitle);
            expect(
              score,
              "Числовая оценка должна быть скрыта (enableOnlyCustomCharacteristics=true)",
            ).toBeNull();

            console.log(
              `  Label="${label}" (ожидалось "${charLabel}"), Score=${score} (null = корректно)`,
            );
          },
        );
