...
test.describe(
  "Исключение: Аватар на вкладке «Респонденты» НЕ ведёт в профиль",
  ...
  () => {
    ...
    test.beforeEach(async ({ adminAuth: page, request }, testInfo) => {
      ...
      await prConfigPage.goToRespondentsTab();
    });

    test(
      "C7498: Клик по аватару респондента на вкладке «Заполнение анкет» не переходит в профиль",
      { tag: ["@critical"] },
      async ({ adminAuth: page }) => {
        setSeverity("critical");

        await test.step("Найти первый аватар в таблице «Респонденты»", async () => {
        ...
        await test.step("Проверить, что НЕ произошёл переход в профиль", async () => {
          await expect(
            page,
            "Клик по аватару респондента НЕ должен переходить в /profile/",
