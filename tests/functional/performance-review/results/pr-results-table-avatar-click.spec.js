...
test.describe(
  "Результаты оценки — Переход в профиль через аватар в таблице",
  ...
  () => {
    ...
    test(
      "C7484: Клик по аватару оцениваемого в таблице результатов открывает профиль",
      { tag: ["@critical"] },
      async ({ adminAuth: page }) => {
        setSeverity("critical");

        await test.step("Кликнуть по аватару оцениваемого", async () => {
          await prConfigPage.clickEmployeeAvatarInResults(evaluatedUserName);
        });
