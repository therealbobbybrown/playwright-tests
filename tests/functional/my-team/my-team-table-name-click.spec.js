...
test.describe(
  "Моя команда — Переход в профиль через имя в таблице",
  ...
  () => {
    ...
    test(
      "C7472: Клик по имени сотрудника в таблице открывает его профиль",
      { tag: ["@critical"] },
      async ({ managerAuth: page }, testInfo) => {
        ...
        await test.step("Кликнуть по имени сотрудника", async () => {
          await myTeamPage.clickEmployeeName(employeeName);
        });
