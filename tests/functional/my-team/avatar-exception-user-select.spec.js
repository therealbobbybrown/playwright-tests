...
test.describe(
  "Исключение: Аватар в селекте пользователей НЕ ведёт в профиль",
  ...
  () => {
    ...
    test(
      "C7460: Клик по аватару в селекте пользователей выбирает сотрудника, а не открывает профиль",
      { tag: ["@critical"] },
      async ({ adminAuth: page, request }, testInfo) => {
        ...
        await test.step("Найти или создать draft PR через API", async () => {
          ...
          const draftPR = prItems.find(
            (pr) => pr.status === "draft" || pr.status === "running",
          );
          ...
          if (draftPR) {
            ...
            await seed.init();
            const created = await seed.seedDraftPR({ title: "Avatar user-select test draft" });
            prId = created.id;
          }
        });

        await test.step("Открыть PR и перейти к выбору участников", async () => {
          await page.goto(
        ...
        await test.step("Проверить, что НЕ произошёл переход в профиль", async () => {
          await expect(
            page,
            "Клик в селекте НЕ должен переходить в /profile/",
